...
- **AI.Foundry.Local.RAG.AppHost**: The Aspire orchestrator project that coordinates all services. Uses the latest `Aspire.AppHost.Sdk` SDK. Entry point is `AppHost.cs` which creates a minimal `DistributedApplication` with no resources configured by default.

- **AI.Foundry.Local.RAG.ServiceDefaults**: A shared library project that provides common Aspire service configuration (OpenTelemetry, service discovery, resilience, health checks). This project should be referenced by all service projects in an Aspire application. The main extension methods are in `Extensions.cs`.
...
Build the solution:
```
dotnet build src/AI.Foundry.Local.RAG.slnx
```

Run the AppHost (starts the Aspire dashboard and orchestrates resources):
```
dotnet run --project src/AI.Foundry.Local.RAG.AppHost
```

The AppHost will launch the Aspire Dashboard at the URLs configured in `launchSettings.json` (default: https://ai_foundry_local_rag.dev.localhost:17232).
